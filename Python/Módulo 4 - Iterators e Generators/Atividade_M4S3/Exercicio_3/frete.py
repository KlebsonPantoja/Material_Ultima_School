def ler_dimensoes_objeto():
    ...
    while not lido_sucesso:
        ...
        if volume > 100000 or volume <= 0:
            print('Não aceitamos objetos com grandes dimensões e/ou zerados')
...
def ler_peso_objeto():
    ...
    while not lido_sucesso:
        peso_lido = input('Dígito do peso do objeto (em Kg): ')
        try:
            peso = float(peso_lido)
            if peso > 30:
                print('Não são aceitos objetos tão pesados')
...
def ler_rota():
    ...
    while not lido_sucesso:
        ...
        rota = input('Digite a rota desejada (utilize como siglas): ')
        rota = rota.lower()
